...
# --------- Step 3: Simplified Neural Network (FNO-inspired) ---------
class SimpleFNO(nn.Module):
    def __init__(self, width, in_features):
        ...
        # Spectral layers - we'll use simple MLP to mimic FNO behavior
    ...
    def forward(self, x):
        ...
        # First spectral-like layer with skip connection
        x1 = self.spectral1(x)
        x2 = self.w1(x)
        x = F.gelu(x1 + x2)
        x = self.dropout(x)
        
        # Second spectral-like layer with skip connection
        x1 = self.spectral2(x)
        x2 = self.w2(x)
        x = F.gelu(x1 + x2)
        x = self.dropout(x)
...
model = SimpleFNO(width=64, in_features=12).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
loss_fn = nn.MSELoss()
checkpoint_path = "fno_3body.pth"
start_epoch = 0
loss_history = []
best_loss = float('inf')
...
axs[1].legend(); axs[1].set_title("Model Prediction")
